...
let session = await client.login(credential); 
  if (session.isLoggedIn) {
    ...
    store.add(THIS("post01"), RDF("type"), FAIRNET('Post'), postsDoc);
    store.add(THIS("post01"), FAIRNET("title"), 'Title-01', postsDoc);
    store.add(THIS("post01"), FAIRNET("content"), "Content-01", postsDoc);
    store.add(THIS("post01"), FAIRNET("ownedBy"), me, postsDoc);

    store.add(THIS("post02"), RDF("type"), FAIRNET('Post'), postsDoc);
    store.add(THIS("post02"), FAIRNET("title"), 'Title-02', postsDoc);
    store.add(THIS("post02"), FAIRNET("content"), "Content-02", postsDoc);
    store.add(THIS("post02"), FAIRNET("ownedBy"), me, postsDoc);
